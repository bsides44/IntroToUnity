...
namespace Unity.ContentManager
{
    ...
    {
        internal const string ContentProductLabel = "__content_product";
        internal const string ContentPackLabel = "__content_pack";

        const string k_ProductSearchFilter = "l:" + ContentProductLabel;
        const string k_CollectionSearchFilter = "l:" + ContentPackLabel;
        ...
        static readonly Type k_ImportWindowType = typeof(EditorWindow).Assembly.GetType("UnityEditor.PackageImport");
        static readonly MethodInfo k_ImportWindowMethod = typeof(EditorWindow).GetMethod("HasOpenInstances", BindingFlags.Static | BindingFlags.Public)?.MakeGenericMethod(k_ImportWindowType);
        ...
        static ContentManagerDriver()
        {
            if (k_ImportWindowType != null)
                k_ImportWindowMethod = typeof(EditorWindow).GetMethod("HasOpenInstances", BindingFlags.Static | BindingFlags.Public)?.MakeGenericMethod(k_ImportWindowType);
        }
        ...
        internal static void InstallContent(ContentPack contentToInstall)
        {
            ...
            var isTgz = contentToInstall.Url.EndsWith(k_TgzExtension);
            QueuedEvents.Add(new ContentEvent
            {
                action = isTgz ? PackageAction.Download : PackageAction.Add,
                targetPack = contentToInstall
            });
        ...
        /// <summary>
        /// Attempts to download a .tgz package specified within the currently active Content Pack.
        /// </summary>
        static void StartDownloadEvent()
        {
            // Is the package already installed? Do nothing
            // Otherwise, start a download action
            var targetPack = s_CurrentEvent.targetPack;
            var installStatus = targetPack.InstallStatus;
            if (installStatus.HasFlag(InstallationStatus.Installed) && !installStatus.HasFlag(InstallationStatus.DifferentVersion))
            {
                OnInstallContent?.Invoke(true, $"{targetPack.PackageName} already installed");
                s_CurrentEvent = null;
                return;
            }
            ...
            var webRequest = new UnityWebRequest(targetPack.Url);
            var fileName = Path.Combine(k_ContentPackagesFolder, $"{targetPack.PackageName}{k_TgzExtension}");
            ...
            {
                var request = Client.Add($"file:../{fileName}");
                s_CurrentEvent = new ContentEvent {action = PackageAction.Add, request = request, targetPack = targetPack};
                OnContentChanged?.Invoke();
            };
#else
            Debug.LogWarning($"Content manager cannot download {targetPack.DisplayName} without the " +
                "UnityWebRequest Module. Please add the UnityWebRequest built-in module via Package Manager");
#endif
        }
        ...
        {
            // If the import window is open - we don't progress until it is closed
            if (k_ImportWindowMethod != null && k_ImportWindowMethod.Invoke(null, null).Equals(true))
                s_CurrentEvent.EventComplete = false;
            else
                s_CurrentEvent.EventComplete = true;
        }

        static void CompleteSampleEvent()
        {
            var targetPack = s_CurrentEvent.targetPack;
        ...
        static void ContentRequestUpdate()
        {
            ...
            else
            {
                ...
                if (s_CurrentEvent.EventComplete)
                {
                    // Complete the action
                    switch (s_CurrentEvent.action)
                    {
