...
namespace Unity.XRTools.Utils
{
    ...
    public abstract class ScriptableSettings<T> : Internal.ScriptableSettingsBase<T> where T : ScriptableObject
    {
...
namespace Unity.XRTools.Utils.Internal
{
    ...
    public abstract class ScriptableSettingsBase<T> : ScriptableSettingsBase where T : ScriptableObject
    {
        ...
        protected ScriptableSettingsBase()
        {
            if (s_Instance != null)
            {
                XRToolsLoggingUtils.LogError(string.Format("ScriptableSingleton {0} already exists. This can happen if there are " +
        ...
        protected static void Save(string savePathFormat)
        {
            ...
            if (s_Instance == null)
            {
                XRToolsLoggingUtils.Log("Cannot save ScriptableSettings: no instance!");
                return;
            }
            ...
            if (k_HasCustomPath)
            {
                ...
                if (ValidatePath(pathAttribute.path, out path))
                {
                    ...
                    {
                        XRToolsLoggingUtils.LogWarning($"{k_PathExceptionMessage}\n{e}");
                        generatePath = true;
                    }
                }

                if (generatePath)
                    XRToolsLoggingUtils.LogWarning($"The path '{pathAttribute.path}' is invalid. Generating a path instead.");
            }
            ...
            if (generatePath)
            {
                ...
                if (scriptData == null)
                {
                    XRToolsLoggingUtils.LogWarning(
            ...
            XRToolsLoggingUtils.Log(string.Format("Created initial copy of settings: {0} at {1}", GetFilePath(), savePath));
#endif
        }
