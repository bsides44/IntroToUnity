        ...
        internal class ModuleRow
        {
            ...
            public MonoScript script
            {
                get
                {
                    ...
                    if (!m_TriedToLoadMonoScript)
                    {
                        m_TriedToLoadMonoScript = true;
                        var scripts = AssetDatabase.FindAssets(type.Name, k_SearchInFolders);
                        foreach (var guid in scripts)
                        {
                            var importer = AssetImporter.GetAtPath(AssetDatabase.GUIDToAssetPath(guid)) as MonoImporter;
                            if (importer)
                            {
                                var importedScript = importer.GetScript();
                                var importedScriptType = importedScript.GetClass();
                                if (importedScriptType == null || importedScriptType != type)
                                    continue;

                                m_Script = importedScript;
                            }
                        }
                    }
            ...
            if (m_ModuleRows.TryGetValue(moduleType, out row))
            {
                using (new EditorGUI.DisabledScope(true))
                {
                    ...
                    if (script)
                        ...
                        EditorGUILayout.LabelField(row.readableTypeName);
                }
            }
            else
            {
