...
namespace Unity.MARS
{
    ...
    public class MARSSession : MonoBehaviour, IUsesFunctionalityInjection
    {
        ...
        const string k_NotEntitledMessageWithUrl = NotEntitledMessage + " Please go to " + LicensingUrl;
        const string k_EntitlementsFailedMessage = "MARS features are disabled in Play mode. " + k_NotEntitledMessageWithUrl;
...
#pragma warning restore 649
        ...
        void Awake()
        {
            ...
            if (sessionCount != 1)
                Debug.LogError("More than one MARSSession in the scene or a scene with an active MARSSession was loaded while an existing MARSSession was active. MARS will not function properly");
            else
            {
                CheckCapabilities();
                Instance = this;
        ...
        {
#if UNITY_EDITOR
            if (Application.isPlaying && !EditorOnlyDelegates.HasEntitlements())
                Debug.LogWarning(k_EntitlementsFailedMessage);
#endif
        ...
        void Start()
        {
            ...
            {
                var requested = m_RequestedMaximumFaceCount >= 0 ? m_RequestedMaximumFaceCount : Int32.MaxValue;
        ...
        static void EnsureSessionConfigured(MARSSession session, UndoBlock undoBlock, Transform overrideUserRef = null)
        {
            ...
            if (!TestMode && userRef == null)
            {
                ...
                if (userRef == null)
                {
                    userRef = overrideUserRef == null ? GameObjectUtils.Instantiate(MARSRuntimePrefabs.instance.UserPrefab).transform : overrideUserRef;
