...
namespace Unity.MARS.Data.Recorded
{
    ...
    public class CameraPoseRecorder : DataRecorder, IUsesCameraPose, IUsesSlowTasks
    {
        ...
        public override DataRecording TryCreateDataRecording(TimelineAsset timeline, List<UnityObject> newAssets)
        {
            ...
            // End at xPosCurve.length to avoid out of range errors.
            // A curve.length can be less than eventsCount if there are keyFrames with same time (in this case the added keyFrame overrides the previous one).
            for (var i = 2; i < xPosCurve.length; i++)
            {
