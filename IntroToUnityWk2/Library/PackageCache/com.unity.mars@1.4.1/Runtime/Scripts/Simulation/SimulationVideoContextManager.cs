...
namespace Unity.MARS.Simulation
{
    class SimulationVideoContextManager : MonoBehaviour, IModuleBehaviorCallbacks, IProvidesSimulationVideoFeed, IUsesCameraPose
    {
        ...
        {
#if UNITY_EDITOR
            if (EditorOnlyDelegates.KeepDataBetweenSimulations != null && EditorOnlyDelegates.KeepDataBetweenSimulations())
            {
                m_KeepVideoPlaying = true;
            }
            else
#endif
            if (!m_KeepVideoPlaying)
