...
#pragma warning restore 649
        ...
        void EnsureInitialized()
        {
            ...
            if (!Application.isPlaying || sceneModule == null || !sceneModule.simulateInPlaymode)
                return;

            var compositeRenderBackground = GetComponent<CompositeRenderBackground>();
            if (compositeRenderBackground == null)
                compositeRenderBackground = gameObject.AddComponent<CompositeRenderBackground>();

            var sessionCameraDescriptor = new RenderTextureDescriptor(MarsCamera.pixelWidth, MarsCamera.pixelHeight);
            var compositeData = new CompositeRenderData(CompositeViewType.GameView, MarsCamera, sessionCameraDescriptor,
                Color.black, SimulationConstants.SimulatedEnvironmentLayerMask, true, false, false, true);
        ...
        void OnDisable()
        {
            ...
            var compositeRenderBackground = GetComponent<CompositeRenderBackground>();
            if (compositeRenderBackground == null)
                UnityObjectUtils.Destroy(compositeRenderBackground);
#endif
        }
