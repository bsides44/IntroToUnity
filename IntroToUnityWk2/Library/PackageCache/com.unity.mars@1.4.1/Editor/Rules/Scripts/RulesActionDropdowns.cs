...
namespace Unity.MARS.Rules.RulesEditor
{
    class ProxyActionsDropdown : AdvancedDropdown
    {
        ...
        const string k_MaterialLabel = "Build Surface (Paint)";
        const string k_ColliderLabel = "Build Surface (Collider)";
        ...
        protected override AdvancedDropdownItem BuildRoot()
        {
            ...
            root.AddChild(new ActionsDropdownItem(k_MaterialLabel, () =>
            {
                RulesModule.Pick<Material>(mat =>
                {
                    ...
                    var buildSurfaceObject = RulesModule.CreateBuildSurfacePaint(mat);
                    RulesModule.NewObject = buildSurfaceObject.transform;
                });
            }));

            root.AddChild(new ActionsDropdownItem(k_ColliderLabel, () =>
            {
                var buildSurfaceObject = RulesModule.CreateBuildSurfaceCollider();
                RulesModule.NewObject = buildSurfaceObject.transform;
                RulesModule.ReParentNewObject();

                RulesModule.AfterHandlePick(() =>
                {
