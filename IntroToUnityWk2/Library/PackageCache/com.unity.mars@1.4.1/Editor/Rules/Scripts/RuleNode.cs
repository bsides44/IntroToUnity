...
namespace Unity.MARS.Rules.RulesEditor
{
    abstract class RuleNode : BindableElement
    {
        ...
        protected VisualElement NodeContainer;
        protected VisualElement AddButtonHoverElement;
        Button m_AddButton;
        ...
        internal static RuleNode CreateRow(UnityObject target)
        {
            if (target == null)
                return null;

            var rowType = CustomRuleNodeAttribute.GetRuleNodeType(target);
            if (rowType == null)
                return null;

            var row = (RuleNode)Activator.CreateInstance(rowType, true);
            row.Init(target);
            return row;
        }
        ...
        protected UnityObject Target { get; private set; }

        internal virtual Transform ContainerObject => null;
        internal virtual GameObject BackingObject => null;

        void Init(UnityObject target)
        {
            Target = target;
        ...
        {
            NodeContainer = this.Q<VisualElement>(k_RuleNodeContainerName);

            NodeContainer.RegisterCallback<MouseDownEvent>(OnMouseDown);
            NodeContainer.RegisterCallback<MouseMoveEvent>(OnMouseMove);
            NodeContainer.RegisterCallback<MouseUpEvent>(OnMouseUp);
            ...
            {
                AddButtonHoverElement = addButtonTemplate.Q<VisualElement>(k_AddButtonHoverElementName);
                m_AddButton = addButtonTemplate.Q<Button>(k_AddButtonName);

                AddButtonHoverElement.RegisterCallback((MouseOverEvent evt, Button addButton) =>
                {
                    addButton.style.display = DisplayStyle.Flex;
                }, m_AddButton);

                AddButtonHoverElement.RegisterCallback((MouseOutEvent evt, Button addButton) =>
                {
        ...
        protected void Select(UnityObject obj, Transform viewObject, GameObject selectedRuleNode)
        {
            ...
            RulesModule.DisplayElementSelected(NodeContainer);
