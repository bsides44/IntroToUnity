...
namespace UnityEditor.MARS.Simulation
{
    ...
    public class HierarchyTreeView : TreeView
    {
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            var rowItemObj = EditorUtility.InstanceIDToObject(args.item.id);
            var itemIsPartOfPrefab = rowItemObj != null && PrefabUtility.IsPartOfPrefabInstance(rowItemObj);

            Color rowColor;
            if (args.selected)
                rowColor = Color.white;
            else if(m_IsEnvironmentHierarchy)
                rowColor = itemIsPartOfPrefab ? MarsEditorGUI.Styles.BlueText : GetTextColorForRow(args);
            else
                rowColor = GetTextColorForRow(args);
            
            styles.rowLabelStyle.normal.textColor = rowColor;
