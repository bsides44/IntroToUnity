...
namespace Unity.MARS
{
    static class CreateMenuItems
    {
        ...
        const string k_CreateMeshVisualizerLabel = k_VisualizersPrefab + "Mesh Visualizer";
        ...
        [MenuItem(k_CreateMeshVisualizerLabel, false, k_VisualizersPriority)]
        public static MarsMeshVisualizer CreateMeshVisualizer(MenuCommand command)
        {
            GameObject createdObj = null;
            var parent = command.context as GameObject;
            var parentTransform = parent != null ? parent.transform : null;

            if (MarsObjectCreationResources.instance.MeshVisualsPreset.CreateGameObject(out createdObj, parentTransform))
            {
                EditorEvents.CreationMenuItemUsed.Send(new MarsMenuEventArgs { label = k_CreateMeshVisualizerLabel });
            }

            return createdObj == null ? null : createdObj.GetComponent<MarsMeshVisualizer>();
        }
