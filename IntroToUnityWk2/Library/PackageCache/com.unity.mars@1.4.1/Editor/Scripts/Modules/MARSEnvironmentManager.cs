...
namespace UnityEditor.MARS.Simulation
{
        ...
        IModuleDependency<SimulationVideoContextManager>, IProvidesDeviceSimulationSettings
    {
        ...
        const string k_EnabledPropertyName = "m_Enabled";
        ...
        static readonly List<Renderer> k_AllRenderers = new List<Renderer>();
...
#pragma warning restore 649
        ...
        void SetupEnvironment()
        {
            ...
            switch (SimulationSettings.instance.EnvironmentMode)
            {
                case EnvironmentMode.Synthetic:
                    ...
                    var environmentPrefabPath = AssetDatabase.GUIDToAssetPath(simulationSettings.SyntheticModeEnvironmentGuid);
                    if (environmentPrefabPath != null)
                    {
                        var newEnvironmentPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(environmentPrefabPath);
                        if (newEnvironmentPrefab != null && newEnvironmentPrefab != simulationSettings.EnvironmentPrefab)
                        {
                            var newEnvironmentPrefabIndex = EnvironmentPrefabNames.IndexOf(newEnvironmentPrefab.name);
                            if (newEnvironmentPrefabIndex >= 0)
                                SetSyntheticEnvironment(newEnvironmentPrefabIndex);
                        }
                    }
                ...
                case EnvironmentMode.Recorded:
                    ...
                    if (recording == null || !Entitlements.IsEntitled)
                        break;
                    ...
                    else if (recording.HasSyntheticEnvironments)
                    {
                        simulationSettings.UseSyntheticRecording = true;
                        SetSyntheticRecordingAndEnvironment(recording);
                        OpenSyntheticEnvironment();
                    }
                    break;
                case EnvironmentMode.Live:
                    var deviceCount = WebCamTexture.devices.Length;
                    if (deviceCount <= 0 || !Entitlements.IsEntitled)
                    {
                        if (simulationSettings.WebCamDeviceIndex >= 0)
        ...
        void RevertIgnoredModifications()
        {
            ...
            foreach (var objOverride in overrides)
            {
                ...
                else if (IsObjectSimDataRenderer(objOverride.instanceObject))
                {
                    var prop = serializedObject.FindProperty(k_EnabledPropertyName);
                    if (prop != null && prop.prefabOverride)
                    {
                        prop.prefabOverride = false;
                        needsApply = true;
                ...
                if (property != null)
                {
                    ...
                    for (var i = size - 1; i > -1; i--)
                    {
                        ...
                        if (targetProp != null && pathProp != null && valueProp != null)
                        {
                            ...
                            if (targetObject != null && IsObjectSimDataRenderer(targetObject) &&
                                pathProp.stringValue.Contains(k_EnabledPropertyName))
                            {
                                property.DeleteArrayElementAtIndex(i);
                                break;
                            }
        ...
        internal bool HasSaveableEnvironmentPrefabInstanceOverrides()
        {
            if (PrefabUtility.HasPrefabInstanceAnyOverrides(CurrentLoadedPrefabInstanceEnvironment, false))
            {
                ...
                foreach (var propertyModification in propertyModifications)
                {
                    ...
                    else if (IsObjectSimDataRenderer(propertyModification.target) &&
                             propertyModification.propertyPath.Contains(k_EnabledPropertyName))
                    {
                        isIgnoreProp = true;
                    }
                    else
        ...
        void UpdateEnvironmentModes()
        {
            s_ModeTypes = Enum.GetNames(typeof(EnvironmentMode))
                ...
                .Select(k => k.Replace(EnvironmentMode.Synthetic.ToString(),"Synthetic Environment"))
                .Select(k => k.Replace(EnvironmentMode.Recorded.ToString(), "Recordings"))
                .Select(k => k.Replace(EnvironmentMode.Live.ToString(),"Video Feed"))
        ...
        void UpdateSimulationSettingsToNextEnvironment(bool forward)
        {
            ...
            switch (simulationSettings.EnvironmentMode)
            {
                case EnvironmentMode.Synthetic:
                    ...
                    {
                        var newRecording = forward
                            ? m_SimulationRecordingManager.GetNextSyntheticRecording()
                            : m_SimulationRecordingManager.GetPreviousSyntheticRecording();
                        if (newRecording == null)
                            break;

                        SetSyntheticRecordingAndEnvironment(newRecording);
                    }
                    else
                    {
        ...
        static IEnumerable<Renderer> GetSimRenderers(SimulatedRendererType seeking)
        {
            ...
            instance.EnvironmentParent.GetComponentsInChildren(k_AllRenderers);
            instance.EnvironmentParent.GetComponentsInChildren(k_PlanesRoots);
            foreach (var planesRoot in k_PlanesRoots)
            {
                planesRoot.GetComponentsInChildren(k_PlaneRenderers);
                foreach (var planeRenderer in k_PlaneRenderers)
                {
                    // Renderers for generated planes are always disabled in the environment scene
                    k_AllRenderers.Remove(planeRenderer);
                }

                k_PlaneRenderers.Clear();
            }

            foreach (var renderer in k_AllRenderers)
            {
                // Renderers on synthesized data are visualizing invisible data,
                // and not actually seen as part of the environment
                var synthesizedRenderer = HasSynthesizedComponent(renderer);
            ...
            k_AllRenderers.Clear();
            k_PlanesRoots.Clear();
            return results;
        }

        static bool IsObjectSimDataRenderer(UnityEngine.Object obj)
        {
            return obj is Renderer renderer && HasSynthesizedComponent(renderer);
        }

        static bool HasSynthesizedComponent(Component component)
        {
            return component.GetComponent<SynthesizedTrait>() != null || component.GetComponent<SynthesizedTrackable>() != null;
        }
        ...
        void SetSyntheticRecordingAndEnvironment(SessionRecordingInfo recording)
        {
            k_TempGameObjects.Clear();
            recording.GetSyntheticEnvironments(k_TempGameObjects);
            if (k_TempGameObjects.Count <= 0 || k_TempGameObjects[0] == null)
                return;

            var newEnvironmentPrefab = k_TempGameObjects[0];
            k_TempGameObjects.Clear();
            var newEnvironmentPrefabIndex = EnvironmentPrefabNames.IndexOf(newEnvironmentPrefab.name);
            if (newEnvironmentPrefabIndex < 0)
                return;

            SimulationSettings.instance.SetRecordingForEnvironment(newEnvironmentPrefab, recording);
            if (newEnvironmentPrefabIndex != CurrentSyntheticEnvironmentIndex)
                SetSyntheticEnvironment(newEnvironmentPrefabIndex);
        }
    }
}
