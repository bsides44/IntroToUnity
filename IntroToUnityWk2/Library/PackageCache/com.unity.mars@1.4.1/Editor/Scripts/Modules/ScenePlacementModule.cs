...
namespace UnityEditor.MARS.Authoring
{
    ...
    public class ScenePlacementModule : IModuleDependency<SceneViewInteractionModule>, IModuleDependency<SimulationSceneModule>
    {
        ...
        public bool orientToSurface
        {
            get
            {
                if (m_UsePlacementOverrides && PlacementOverrides.useOrientToSurfaceOverride)
        ...
        public bool snapToPivot
        {
            get
            {
                if (m_CurrentDraggedObject && PlacementOverrides.useSnapToPivotOverride)
        ...
        public AxisEnum orientAxis
        {
            get
            {
                return m_CurrentDraggedObject && PlacementOverrides.useAxisOverride
        ...
        Vector3 OrientAxisDirection
        {
            get
            {
                return m_UsePlacementOverrides && PlacementOverrides.useAxisOverride
        ...
        void UpdatePlacementOverrides()
        {
            ...
            m_UsePlacementOverrides = false;
            PlacementOverrides.ResetData();
            var redirectionSelection = m_SceneViewInteraction.CurrentHoverTarget.GetComponent<RedirectSelection>();
            if (redirectionSelection != null && redirectionSelection.target != null)
            {
                var targetPlacementOverride = redirectionSelection.target.GetComponent<PlacementOverride>();
                if (targetPlacementOverride != null)
                {
                    m_UsePlacementOverrides = true;
        ...
        internal void EnsureDraggedObjectInSimViewIsInContent()
        {
            // Only move object to content scene if it is in simulation (in environment scene)
            if (m_CurrentDraggedObject != null && m_CurrentDraggedObject.scene == m_SimulationSceneModule.EnvironmentScene &&
                m_CurrentDraggedObject.scene != m_SimulationSceneModule.ContentScene)
            {
                m_SimulationSceneModule.AddContentGameObject(m_CurrentDraggedObject, true);
            }
        }
    }
}
