...
namespace UnityEditor.MARS.Simulation
{
        ...
        IModuleDependency<SceneWatchdogModule>
    {
        ...
        UndoPropertyModification[] OnPostprocessModifications(UndoPropertyModification[] modifications)
        {
            ...
            {
                // Live-update transform changes to children of proxies
                var transformTarget = modification.currentValue.target as Transform;
                if (transformTarget != null && transformTarget.GetComponentInParent<Proxy>() != null &&
                    transformTarget.GetComponent<Proxy>() == null &&
                    transformTarget.GetComponentInParent<Replicator>() == null) // cannot live-update replicated content
                {
                    var transformCopy = GetCopiedTransform(transformTarget);
                    if (transformCopy != null)
                    {
                        var targetParent = transformTarget.parent;
                        var copyParent = transformCopy.parent;
                        if (targetParent != null && copyParent == GetCopiedTransform(targetParent))
                        {
                            transformCopy.localPosition = transformTarget.localPosition;
                            transformCopy.localRotation = transformTarget.localRotation;
                            transformCopy.localScale = transformTarget.localScale;
                            continue;
                        }
                    }
                }
                ...
                // Need to check both GetComponentInParent and GetComponent because GetComponentInParent ignores inactive GameObjects
                var isSimulatableObject = hasGameObject &&
                    (gameObject.GetComponent<ISimulatable>() != null || gameObject.GetComponentInParent<ISimulatable>() != null);

                if (isSimulatableObject && OnSimulatableObjectChanged(gameObject))
                    continue;
                ...
                if (transformTarget == null || marsSession == null || transformTarget.gameObject != marsSession.gameObject)
                    continue;
