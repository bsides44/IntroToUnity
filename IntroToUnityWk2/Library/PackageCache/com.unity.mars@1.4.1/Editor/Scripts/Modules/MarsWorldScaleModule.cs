...
#pragma warning restore 649
        ...
        public float AdjustWorldScale(float scale)
        {
            ...
            HandleAudioScaling(m_EnvironmentAudioSources, m_EnvironmentReverbZones, scaleFactor, true);
            HandleLightScaling(m_EnvironmentLights, scaleFactor, true);

            if (settings.scaleSceneAudio)
                HandleAudioScaling(k_AudioSources, k_ReverbZones, scaleFactor, true);

            if (settings.scaleSceneLighting)
                HandleLightScaling(k_SceneLights, scaleFactor, true);
        ...
        public void ApplyWorldScaleToEnvironment()
        {
            ...
            HandleAudioScaling(m_EnvironmentAudioSources, m_EnvironmentReverbZones, scaleFactor, false);
            HandleLightScaling(m_EnvironmentLights, scaleFactor, false);
        }
        ...
        static void HandleLightScaling(List<Light> lights, float scaleFactor, bool recordUndo)
        {
        ...
        static void HandleAudioScaling(List<AudioSource> audioSources, List<AudioReverbZone> reverbZones, float scaleFactor, bool registerUndo)
        {
